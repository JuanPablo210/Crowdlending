...
import {UsuarioRole} from '../../../core/models/data.interface';
...
import {UsuarioRoleFormComponent} from "./usuario-role-form/usuario-role-form.component";
import {RoleDeleteComponent} from "../role/role-delete/role-delete.component";
import {UsuarioRoleDeleteComponent} from "./usuario-role-delete/usuario-role-delete.component";

@Component({
  selector: 'app-usuario-role',
  templateUrl: './usuario-role.component.html',
  styleUrls: ['./usuario-role.component.sass']
})
export class UsuarioRoleComponent implements OnInit {
  public _datos = { _title: 'Usuario Role', _modulo: 'Catalogos', _icono: 'fas fa-folder-open', _dominio: 'UsuarioRole', _componente: 'Usuario Role'};
  displayedColumns = [ 'select', 'usuario',
  'role', 'actions' ];
  selection = new SelectionModel<UsuarioRole>(true, []);
  advanceTable: UsuarioRole | null;
  ...
  addNew() {
    let data: any;
    this.advanceTableService.create<UsuarioRole>().subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(UsuarioRoleFormComponent, {
  ...
  editCall(row) {
    this.id = row.id;
    let data: any;
    this.advanceTableService.edit<UsuarioRole>(this.id).subscribe(result => {
      data = result;
      console.log(this.advanceTable);
      const dialogRef = this.dialog.open(UsuarioRoleFormComponent, {
  ...
  deleteItem(row) {
    this.id = row.id;
    const dialogRef = this.dialog.open(UsuarioRoleDeleteComponent, { data: row });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      this.advanceTableService.delete<string>(item.usuario).subscribe();
      this.selection = new SelectionModel<UsuarioRole>(true, []);
    });
    this.loadData();
  ...
  onContextMenu(event: MouseEvent, item: UsuarioRole) {
...
export class DivisaDataSource extends DataSource<UsuarioRole> {
  _filterChange = new BehaviorSubject('');
  get filter(): string { return this._filterChange.value; }
  set filter(filter: string) { this._filterChange.next(filter); }
  filteredData: UsuarioRole[] = [];
  renderedData: UsuarioRole[] = [];
  ...
  connect(): Observable<UsuarioRole[]> {
    ...
    return merge(...displayDataChanges).pipe( map(() => {
        this.filteredData = this._dataSource.data.slice().filter((advanceTable: UsuarioRole) => {
          const searchStr = ( advanceTable.usuario  + advanceTable.role ).toLowerCase();
  ...
  sortData(data: UsuarioRole[]): UsuarioRole[] {
